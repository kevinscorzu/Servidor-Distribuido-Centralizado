...
int main(void) {
    ...
    unsigned char *img = stbi_load("dragon.jpg", &width, &height, &channels, 0);
    unsigned char *img_h = stbi_load("homero.jpg", &width_h, &height_h, &channels_h, 0);
    ...
    for(unsigned char *p = img, *h =img_h,*pg = gray_img; p != img + img_size; p += channels,h+=channels_h, pg += channels) {
        ...
        *pg       = (uint8_t)*p ^ *h;         // red
        *(pg + 1) = (uint8_t)*p ^ *h;         // green
        *(pg + 2) = (uint8_t)*p ^ *h;        
        if(channels == 4) {
            *(pg + 3) = *(p + 3);
        }
    }
    stbi_write_jpg("sky_gray.jpg", width, height, channels, gray_img, 100);
}